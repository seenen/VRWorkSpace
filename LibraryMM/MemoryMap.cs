...
namespace LibraryMM
{
    public class MemoryMap
    {
        ...
        /// 持久映射
        /// </summary>
        public void CreatePersistentMappedFile(string filepath, string mapped_name, long offset, long length)
        {
            filepath = "G:\\GitHub\\VRWorkSpace\\Output\\MemoryMappedFile.data";

            MmfInfo info = new MmfInfo();
            info.mMappedName = mapped_name;
            info.mMappedCapacity = length;
            info.IsPersistent = true;
            info.PersistentFilePath = filepath;

            //using (mMemoryMappedFile = MemoryMappedFile.CreateFromFile(@"c:\内存映射文件.data", FileMode.Open, "公用名"))
            using (var mmf = MemoryMappedFile.CreateFromFile(filepath, FileMode.Open, mapped_name))
            {
        ...
        public void OpenPersistentMappedFile(string mapped_name, long offset, long length)
        {
            //另一个进程或线程可以，在系统内存中打开一个具有指定名称的现有内存映射文件
            using (var mmf = MemoryMappedFile.OpenExisting(mapped_name))
            {
        ...
        public ComOperate CreateNonPersistentMappedFile(string mapped_name, string mutexName, long mapped_capacity = 1024 * 1024)
        {
            ...
            info.mmf = MemoryMappedFile.CreateNew(mapped_name, mapped_capacity, MemoryMappedFileAccess.ReadWrite);

            //using (info.mmf = MemoryMappedFile.CreateNew(mapped_name, mapped_capacity, MemoryMappedFileAccess.ReadWrite))
            {
