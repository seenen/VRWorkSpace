...
namespace LibraryGeometryFormat
{
    public class ObjFile
    {
        ...
        private static ObjModel OrganizeModels(ObjModel model)
        {
            ...
            for (int i = 0; i<model.innerFaceList.Count; i++)
            {
                ...
                var tuple = new Tuple(face.vertex0.position, face.vertex1.position, face.vertex2.position);
                ...
                //result.faceList.Add(new Tuple(face.vertex0.position, face.vertex1.position, face.vertex2.position));
        ...
        private static void GenNormals(ObjModel model)
        {
            ...
            var faceNormals = new _Vector3[model.innerFaceList.Count];
            model.normalList.AddRange(new _Vector3[model.positionList.Count]);

            for (int i = 0; i<model.innerFaceList.Count; i++)
            {
                var face = model.innerFaceList[i];
                _Vector3 vertex0 = model.positionList[face.vertex0.position - 1];
                _Vector3 vertex1 = model.positionList[face.vertex1.position - 1];
                _Vector3 vertex2 = model.positionList[face.vertex2.position - 1];
                _Vector3 v1 = vertex0 - vertex2;
                _Vector3 v2 = vertex2 - vertex1;
                faceNormals[i] = v1.Cross(v2);
            }

            for (int i = 0; i<model.positionList.Count; i++)
            {
                _Vector3 sum = new _Vector3();
                int shared = 0;
            ...
            using (var sr = new StreamReader(filename))
            {
                ...
                while (!sr.EndOfStream)
                {
                    ...
                    if (parts[0] == "v")
                    {
                        ...
                        _Vector3 position = new _Vector3(float.Parse(parts[1]), float.Parse(parts[2]), float.Parse(parts[3]));
                        model.positionList.Add(position);
                    }
                    else if (parts[0] == "vt")
                    {
                        _Vector2 uv = new _Vector2(float.Parse(parts[1]), float.Parse(parts[2]));
                        model.uvList.Add(uv);
                    }
                    else if (parts[0] == "vn")
                    {
                        _Vector3 normal = new _Vector3(float.Parse(parts[1]), float.Parse(parts[2]), float.Parse(parts[3]));
    ...
    public class Tuple
    {
        ...
        public Tuple(int p0index, int p1index, int p2index)
        {
            x = p0index;
            y = p1index;
            z = p2index;
